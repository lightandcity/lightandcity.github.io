(function(modules) {
  ...
  const cookie = (function() {
    const get = (key) => {
      return `; ${document.cookie}`.split(`; ${key}=`)[1]?.split(';')[0] ?? null;
    }
    const has = (key) => {
      return get(key) !== null;
    }
  ...
  const cache = (function() {
    ...
    const add = (recentProducts) => {
      ...
      write([...targetRecentProducts, ...currentRecentProducts, ]);
    }
    return {
      isExpires,
      isEmpty,
  ...
  const renderer = (function() {
    ...
    const render = (products) => {
      ...
      const recentProductsHtml = createContentHtml(createProductItemContainerHtml(products.map(product => createProductItemHtml(product)).join(''), ), );
