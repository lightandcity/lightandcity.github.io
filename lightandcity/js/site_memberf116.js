...
var SITE_MEMBER = function() {
    ...
    var findSubmit = function(step) {
      var that = this;
      if (step == 'find') {
        var status_find_id = !($('._find_id').hasClass('hidden'));
        var status_find_password = !($('._find_password').hasClass('hidden'));
        var email_wrap = $('._find_id_email_wrap');
        var nick_wrap = $('._find_id_nick_wrap');
        var find_id_first_result_wrap = $('#find_step2_info_1');
        var find_id_second_result_wrap = $('#find_step2_info_2');
        var only_email = $('._find_id_only_input_email').val();
        var is_find_id_only_email = false;
        var find_id_send_email_wrap = $('._find_id_send_email_wrap');
        var email = $('._find_id_input_email').val();
        var nick = $('._find_id_input_nick').val();
        var call_num = $('._find_id_input_call_num').val();
        var regex = /[^0-9]/g;
        if (call_num) call_num = call_num.replace(regex, "");
        var find_pw_email = $('._find_pw_input_email').val();
        ...
        if (is_auth_dream_seucrity > 0 || is_auth_mobilians > 0 || is_auth_inicis > 0) {
          is_auth_input = true;
        }
        if (only_email != undefined) {
          is_find_id_only_email = true;
          email = only_email;
        }
        if (status_find_id) {
          find_pw_email = '';
          if (email_wrap.css('display') == 'block') {
            nick = '';
            call_num = '';
          } else if (nick_wrap.css('display') == 'block') {
            email = '';
          }
        ...
        var data = {};
        data.is_find_id_only_email = is_find_id_only_email;
        data.status_find_id = status_find_id;
        data.email = email;
        data.nick = nick;
        ...
        $.ajax({
          ...
          success: function(res) {
            if (res.msg == 'SUCCESS') {
              ...
              $('#token').val(res.token);
              $('#member_code_token').val(res.member_code);
              ...
              } else if ($('#find_step_1').css('display') == 'block' && res.member_data != undefined) {
              ...
              if (res.member_data) {
                ...
                if (res.is_one_member_data) {
                  $.each(res.member_data, function(index, value) {
                    add_member_list.append("<input type='hidden' class='_one_member_encrypt_uid' value='" + value[1] + "' />" + "<input type='hidden' class='_one_member_login_type' value='" + value[2] + "' />" + "<input type='hidden' class='_one_member_asterisk_email' value='" + value[3] + "' />" + "<span class='text-ultra-bold mt16 _result_find_data_uid _one_member_asterisk_uid'>" + value[0] + "</span>");
                  ...
                  if (res.is_use_only_id && !is_auth_input) {
                    ...
                    find_id_second_result_wrap.show();
                    $.each(res.member_data, function(index, value) {
                      add_member_list.append("<div class='pw_search mt8'>" + "<span class='text-ultra-bold mt16 _result_find_data_uid _one_member_asterisk_uid'>" + value[0] + "</span>" + "</div>");
                    });
                  } else {
                    $.each(res.member_data, function(index, value) {
                      add_member_list.append("<div class='select_tit radio radio-styled mt0 mb0 _member_list_radio'>" + "<label>" + "<input type='hidden' class='_asterisk_email' value='" + value[3] + "' />" + "<input type='hidden' value='" + value[2] + "' />" + "<input type='radio' name='findIdMemberList' value='" + value[1] + "' />" + "<span>" + value[0] + "</span>" + "</label>" + "</div>");
                    });
        ...
        $.ajax({
          type: 'POST',
          data: {
            'token': that.findToken,
            'status_find_id': status_find_id,
            'step': 2
          },
          url: ('/ajax/find_pass.cm'),
          dataType: 'json',
    ...
    var joinSubmit = function(is_social) {
      if (join_processing) return false;
      ...
      if ($('.is_change_all_admin').length > 0) {
        $.extend(data, {
          is_change_all_admin: $('.is_change_all_admin').prop('checked')
        });
      }
    ...
    var isDialogOpen = function() {
      if ($.cocoaDialog.isOpen()) $.cocoaDialog.close();
      else window.dozAndroidBridge.dialogClose();
    };
        ...
        $.ajax({
          ...
          success: function(res) {
            if (res.msg === 'SUCCESS') {
              ...
              var isInAppBrowser = userAgent.indexOf('Instagram') > -1 || userAgent.indexOf('FBAN') > -1 || userAgent.indexOf('Line') > -1 || userAgent.indexOf('KAKAOTALK') > -1 || userAgent.indexOf('Kakaotalk') > -1;
    ...
    var updateJoinFormFields = function(data) {
      ...
      if (data.auth_info_save) {
        ...
        if (data.gender) {
          ...
          $('#gender_' + opposite_gender).parent('label').attr('onclick', "alert('본인인증으로 저장된 회원정보는 재인증을 통해서만 수정 가능합니다.')");
    ...
    var openDRMOKWindow = function(req_info, cpid, rtn_url) {
        var step2_select_val;
        var step1_pw_input_val;
        if ($('#token').val() !== undefined) {
          step2_select_val = $('#token').val();
              ...
              success: function(res) {
                if (res.msg === 'SUCCESS') {
                  if (IS_APP || IS_MOBILE) {
                    ...
                    $form.attr("action", urlInfo.protocol + ":
                      ...
                      var $body = $('body'); $body.append($form); $form.submit(); event.returnValue = false; event.cancelBubble = true;
                    }
                    else {
                      windowOpen('DRMOKWindow', 'https:
                      }
                    }
                  }
                });
            };
            var openMOBILIANSWindow = function(CI_SVCID, Tradeid, Okurl, call_back_data) {
                var Keygb = '1';
                var Cryptyn = 'Y';
                var CI_Mode = '61';
                ...
                $.ajax({
                    ...
                    success: function(res) {
                      if (res.msg === 'SUCCESS') {
                        const isAndroidInstagramBrowser = navigator.userAgent.includes('Android') && navigator.userAgent.includes('Instagram');
                ...
                var openINICISWindow = function(mid, Tradeid, Okurl, call_back_data) {
                    var MSTR = call_back_data;
                    ...
                    var openCOMBINATIONAUTHTYPESWindow = function(mid, mTxId, return_url, auth_hash) {
                      var auth_url = 'https:
                      var step2_select_val;
                      var step1_pw_input_val;
                      if ($('#token').val() !== undefined) {
                      ...
                      var openSocialJoin = function(back_url, type_code, social_type) {
                        $.cocoaDialog.close();
                        var third_party_agree = $('#third_party_agree').is(':checked') ? 'Y' : 'N';
                        var marketing_sms_agree = $('#marketing_sms_agree').is(':checked') ? 'Y' : 'N';
                        var marketing_email_agree = $('#marketing_email_agree').is(':checked') ? 'Y' : 'N';
                      ...
                      var initFindId = function(obj) {
                        $('input:radio[name=findIdStep01]').change(function() {
                          ...
                          var find_id_step_01_btn_wrap = $('._find_id_step_01_btn_wrap');
                          ...
                          switch (checked_radio_val) {
                            case 'find_id_email':
                              $('._step01_find_id_email_input_wrap').show();
                              $('._find_id_input_nick').val('');
                              $('._find_id_input_call_num').val('');
                            ...
                            case 'find_id_name_and_phone':
                              $('._step01_find_id_name_and_phone_input_wrap').show();
                              $('._find_id_input_email').val('');
                              find_id_step_01_btn_wrap.append("<button class='btn btn-primary _update_status' data-tab='find_id' onclick=SITE_MEMBER.findSubmit('find');>" + LOCALIZE.버튼_아이디찾기() + "</button>");
                              break;
                        ...
                        $('input:radio[name=findIdStep03]').change(function() {
                          var find_id_step_03_btn_wrap = $('._find_id_step_03_btn_wrap');
                        ...
                        $('._find_id_reset_password').off('click').on('click', function() {
                          current_find_tab = 'findPassword';
                          $('#find_step_1').hide();
                          $('#find_step_2').hide();
                          $('#find_step_3').show();
                          if ($('._member_list_radio').css('display') == 'block') {
                            var check_member_list_radio = $('input:radio[name="findIdMemberList"]:checked');
                            var check_member_list_is_email = check_member_list_radio.prev().val();
                          ...
                          } else if ($('._one_member_login_type').length > 0) {
                            if ($('._one_member_asterisk_email').val() == 'null') {
                              $('._find_id_send_email_wrap').hide();
                        ...
                        $('._find_id_detail').off('click').on('click', function() {
                        ...
                        if (obj.use_mobile_auth == 'Y') {
                          $('._result_find_data_uid').text(obj.result_email);
                          $('#find_step_1').hide();
                          $('#find_step_2').show();
                          $('#find_step_3').hide();
                          $('._find_id_detail').hide();
