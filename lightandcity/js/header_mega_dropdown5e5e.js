var HEADER_MEGA_DROPDOWN = function() {
  ...
  var init = function($w, $d, o) {
    ...
    if (typeof $drop_down == 'undefined') return;
    if ($widget.length == 0) return;
    if ($drop_down.length == 0) return;
    code = option.code;
    ...
    align = option.align;
    calculateRowSize();
    $has_child = $widget.find('.sub_menu').length > 0 ? true : false;
    ...
    if (menu_sync && $(window).width() > 768) {
      ...
      if ($more_menu.length > 0) {
        ...
        $menu_items.each(function() {
          if (isBlank($(this).attr('data-code'))) return false;
          _last_code = $(this).attr('data-code');
        });
        var more_list = [];
        var show_list = [];
        var hit_more = false;
        if (_last_code == '') hit_more = true;
      ...
      if ($body.hasClass('admin')) left = left - $('#edit').offset().left;
  ...
  var open = function(e) {
    if ($body.hasClass('admin') && is_header_config) return;
    if ($('#dropdown_' + code).find('._item').length == 0) return;
    if (!$has_child && $(e.currentTarget).find('._main_menu ._more_menu').length == 0) return;
    if (is_slide_down_ing) return;
    if (!is_slide_down) {
      $drop_down.slideDown({
        'duration': 400,
        'easing': 'swing',
