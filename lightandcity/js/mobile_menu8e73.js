var MOBILE_SLIDE_MENU = function() {
  ...
  var runAccordion = function() {
    ...
    var show = function($el) {
      ...
      $el.show().removeClass('collapse').addClass('collapsing')[dimension](0).attr('aria-expanded', true);
      active_list[$el.attr('data-index')] = $el;
      transitioning = 1;
      var complete = function() {
        $el.removeClass('collapsing').addClass('collapse in')[dimension]('');
        transitioning = 0;
      };
      var scrollSize = $.camelCase(['scroll', dimension].join('-'));
      $el.one('bsTransitionEnd', $.proxy(complete, this)).emulateTransitionEnd(350)[dimension]($el[0][scrollSize]);
    };
    var hide = function($el) {
      if (transitioning || !$el.hasClass('in')) return;
      var dimension = 'height';
      $el[dimension]($el[dimension]())[0].offsetHeight;
      $el.addClass('collapsing').removeClass('collapse in').attr('aria-expanded', false);
      ...
      var complete = function() {
        transitioning = 0;
        $el.removeClass('collapsing').addClass('collapse').hide();
        $el.find('ul').hide();
        delete active_list[$el.attr('data-index')];
      };
      $el[dimension](0).one('bsTransitionEnd', $.proxy(complete, this)).emulateTransitionEnd(350);
    };
    if (!set_accordion) {
  ...
  var hideSlide = function() {
    ...
    if (is_fullpage) $body.css('overflow-y', 'hidden');
