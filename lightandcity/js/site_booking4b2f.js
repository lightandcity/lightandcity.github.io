var SITE_BOOKING = function() {
  ...
  var getValidateDay = function() {
    ...
    for (var i in day_list) {
      if (day_list[i].hasClass('on') === true) {
        var key = 0;
        var full_day_cnt = 0;
        for (var i2 = min_day; i2 <= max_day; i2++) {
          key = Number(i) + Number(i2);
          if (day_list[key].hasClass('full_day') === true) {
  ...
  var addBooking = function(callback) {
    ...
    data = $.extend(data, {
      'unselected_end_day': unselected_end_day
    });
