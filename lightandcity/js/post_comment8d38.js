var POST_COMMENT = function() {
  ...
  var commentInit = function(code) {
    ...
    comment_area.off('input keyup keydown paste change', '._comment_nick').on('input keyup keydown paste change', '._comment_nick', function() {
      var input = $(this);
      setTimeout(function() {
        input.limitLength({
          max_byte: 30
    ...
    $("#comment_image_upload_btn").fileupload({
      ...
      limitMultiFileUploadSize: 110000000,
  ...
  var commentAddSub = function(code) {
    ...
    var data = form.serializeObject();
    var tmp_img = {
      'temp_images': sub_comment_image[code]
    };
  ...
  var commentConfirmShow = function(event, code, type, interlock_type, use_interlock_board) {
    ...
    $post_secret_password.off('click', '._confirm').on('click', '._confirm', function() {
      var secret_pass = $post_secret_password.find('input').val();
      $post_secret_password.hide();
      switch (type) {
        case 'show':
          commentShow(code, secret_pass, interlock_type);
          break;
        case 'delete':
          commentDelete(code, secret_pass, interlock_type, use_interlock_board);
          break;
        case 'edit':
          commentEditShow(code, secret_pass, interlock_type);
          break;
      }
    });
    $('body').off('mousedown.post_secret').on('mousedown.post_secret', function(e) {
  ...
  var commentMapConfirmShow = function(event, code, type, interlock_type) {
    ...
    $post_secret_password.off('click', '._confirm').on('click', '._confirm', function() {
      ...
      switch (type) {
        case 'show':
          commentMapShow(code, secret_pass);
          break;
        case 'delete':
          commentMapDelete(code, secret_pass);
          break;
        case 'edit':
          commentEditShow(code, secret_pass, interlock_type);
          break;
