var SITE = function() {
  ...
  var showLightGallery = function(d) {
    ...
    var modalZ = 17000;
    if ($('#cocoaModal').length == 1) modalZ = $('#cocoaModal').css('z-index');
    if (_type == 'gallery') {
      ...
      var $_html = '<div id="lg-temp-gallery">' + '<a href="' + img_url + '"></a>' + '</div>' + '<style>.lg-backdrop {z-index: ' + (modalZ + 1) + '} .lg-outer {z-index: ' + (modalZ + 2) + '}</style>';
      $_body.append($_html);
      $("#lg-temp-gallery").lightGallery();
      $("#lg-temp-gallery a").trigger('click');
    } else if (_type == 'video') {
      var video_url = _data.video_url;
      var loop = _data.loop;
      ...
      var $_html = '<div id="lg-temp-gallery">' + '<a href="' + video_url + '"></a>' + '</div>' + '<style>.lg-backdrop {z-index: 17001} .lg-outer {z-index: 17002}</style>';
      $_body.append($_html);
      var lg_option = {
        youtubePlayerParams: {
          rel: 0,
      ...
      if (loop === 'Y') {
        lg_option.youtubePlayerParams = Object.assign(lg_option.youtubePlayerParams, {
          loop: 1,
          playlist: youtube_id
        });
        lg_option.vimeoPlayerParams = Object.assign(lg_option.vimeoPlayerParams, {
          loop: 1
        });
      }
