var HEADER_CENTER_COLGROUP = function() {
  ...
  var init = function(c, sd) {
    ...
    if (!$section.hasClass('extend')) return;
    data = sd;
    if (isBlank(data)) return;
    if (isBlank(data.center_width)) return;
    if (parseInt(data.center_width) == 0) return;
    if (parseInt(data.center_width) > parseInt(data.left_width)) return;
    if (parseInt(data.center_width) > parseInt(data.right_width)) return;
    ...
    if ($center_col_group.length == 0) return;
