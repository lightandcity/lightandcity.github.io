var SITE_COUPON = function() {
  ...
  const downloadEntireCoupon = () => {
    ...
    if (downloadReadyCouponList.length > 0) {
      $.ajax({
        ...
        data: {
          code_list: downloadReadyCouponList
        },
        dataType: "json",
        async: true,
        cache: false,
          ...
          if (res.msg === "SUCCESS") {
            ...
            if (excluded_list.length !== 0) {
              excluded_list.forEach(list => {
                let ExcludedShowBtn = list.querySelector('._btn_show_excluded_prod_list');
                let ExcludedHideBtn = list.querySelector('._btn_hide_excluded_prod_list');
  ...
  const downloadCouponPack = (couponPackUrls) => {
    $.ajax({
      ...
      data: {
        coupon_pack_urls: couponPackUrls
      },
      dataType: 'json',
      async: true,
      cache: false,
