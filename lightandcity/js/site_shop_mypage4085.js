var SITE_SHOP_MYPAGE = function() {
  ...
  var cancel_order_code = '';
  var auto_cancel_enable = 'N';
  var return_order_code = '';
  var exchange_order_code = '';
  var is_canceling = false;
  var add_member_billing_check = false;
  ...
  var openPopupPrivacy = function(agree_type) {
    ...
    $.cocoaDialog.open({
      type: 'site_payment_privacy',
      custom_popup: $popup_html
    });
  }
  ...
  var showRegularlyDateList = function(regularly_code, is_multiple_prod) {
    $.ajax({
      type: 'POST',
      data: {
        'is_multiple_prod': is_multiple_prod
      },
    ...
    $.ajax({
      ...
      success: function(result) {
        ...
        if (result.msg == 'SUCCESS') {
          ...
          else $order_list_more_button.show();
        } else {
          alert(result.msg);
        }
      }
  ...
  const omsOrderConfirm = async function(orderCode, orderSectionCode) {
    ...
    const {
      data: authData
    } = await axios.get(`/ajax/oms/OMS_auth.cm`);
    ...
    const {
      data: response
    } = await axios.patch(`/ajax/oms/OMS_wrap_customer_v1_orders_{orderCode}_sections_purchase-confirmation.cm?orderCode=${orderCode}`, {
      orderSectionCode
    }, {
      headers: {
  ...
  var cancelOrderSelectProdOrder = function() {
    ...
    if (auto_cancel_enable == 'Y' || refund_price == 0) {
  ...
  var returnOrderLoadRefundPriceData = function() {
    ...
    if ($order_return_form.find('input._prodOrderCheck').length == $order_return_form.find('input._prodOrderCheck:checked').length) {
      ...
      $order_return_form.find('#_include_deliv_price').show();
    }
  ...
  const omsCancelTotalOrder = async function(orderCode) {
    try {
      ...
      const {
        data
      } = await axios.patch(`/ajax/oms/OMS_wrap_customer_v1_orders_{orderCode}_unpaid-deposit-order_cancel.cm?orderCode=${orderCode}`, null, {
        headers: {
          Authorization: `Bearer ${authData.token}`,
        }
      });
  ...
  var exchangeOrderSelectProdOrder = function() {
    if ($order_exchange_form.find("input._prodOrderCheck:not(:checked)").length == 0 && auto_cancel_enable == 'Y') {
      $('#refund_data_wrap').hide();
    } else {
      $('#refund_data_wrap').show();
    }
  ...
  var changeReturnCollectMethod = function(collect_method) {
    ...
    switch (collect_method) {
      ...
      case 'RETURN_PARCEL':
        ...
        if (collect_method == 'RETURN_PARCEL') {
  ...
  var changeExchangeCollectMethod = function(collect_method) {
    if (collect_method == 'RETURN_DESIGNATED') {
    ...
    $.ajax({
      ...
      success: function(res) {
        if (res.msg == 'SUCCESS') {
          ...
          } else window.location.href = '/shop_mypage/?m2=cancel';
        } else {
          alert(res.msg);
        }
      }
  ...
  const omsWithdrawReturnOrder = function({
    qty = 0,
    orderCode,
    orderSectionCode,
    ...
    if (confirm(getLocalizeString(localizeCode, '', defaultString))) {
      ...
      const sectionManageUrl = new URL(`/customer/v1/orders/${orderCode}/sections/revert-return-request`, window.TEST_SERVER === true ? 'https:)
      return fetch('/ajax/oms/OMS_auth.cm', {
        credentials: 'include'
      }).then(res => res.json()).then(json => json.msg === 'SUCCESS' ? json.token : Promise.reject(json.msg)).then(token => fetch(sectionManageUrl.href, {
        method: 'PATCH',
        ...
        if (json.statusCode !== 200) {
          return Promise.reject(typeof window.LOCALIZE[`E_${json.code}`] === 'function' ? window.LOCALIZE[`E_${json.code}`]() : undefined)
        }
        if (!json.data.updateResult) {
          return Promise.reject()
        }
      }).catch(err => alert(err || window.LOCALIZE.타이틀_일시적인서비스장애())).finally(() => window.location.reload());
    }
  }
  const omsWithdrawCancelOrder = function({
    qty = 0,
    orderCode,
    orderSectionCode,
    orderSectionItemCode
    ...
    if (confirm(getLocalizeString('타이틀_취소신청을철회하시겠습니까', '', '취소신청을 철회 하시겠습니까?'))) {
      ...
      const sectionManageUrl = new URL(`/customer/v1/orders/${orderCode}/sections/revert-cancel-request`, window.TEST_SERVER === true ? 'https:)
      return fetch('/ajax/oms/OMS_auth.cm', {
        credentials: 'include'
      }).then(res => res.json()).then(json => json.msg === 'SUCCESS' ? json.token : Promise.reject(json.msg)).then(token => fetch(sectionManageUrl.href, {
        method: 'PATCH',
  ...
  var requestCashReceiptProc = function(q, cash_receipt_type, cash_receipt_value) {
    $.ajax({
      ...
      data: {
        ...
        'cash_receipt_value': cash_receipt_value
      },
      ...
      if ($('input[name=deliv_memo]:checked').val() === "직접입력") {
        ...
        btn_memo_submit.addEventListener('click', () => {
          ...
          if (memo_data.deliv_memo === "") {
            ...
            axios.post('/ajax/regularly/site_update_modify_regularly_address.cm', memo_check_data).then(res => {
              if (res.status === 200) {
                ...
                if (confirm(msg_text)) {
                  axios.post('/ajax/regularly/site_update_modify_regularly_address.cm', memo_data).then(res => {
                    if (res.status === 200) {
                      console.log(res);
                      location.reload();
                    }
  ...
  const changeRegularlyAddressInDropDown = (...props) => {
    ...
    let check_data = {
      ...
      'deliv_call': deliv_call,
      'deliv_postcode': deliv_postcode,
      'deliv_address': deliv_address,
      'deliv_address_detail': deliv_address_detail,
      'request_type': 'check_deliv_price',
      'regularly_code': regularly_code
    };
    let submit_data = {
    ...
    axios.post('/ajax/regularly/site_update_modify_regularly_address.cm', check_data).then(res => {
      if (res.status === 200 && res.data.msg === 'SUCCESS') {
        const msg_split = res.data.confirm_msg.split(' ', );
        const variant_text = getLocalizeString('설명_'.concat(msg_split[1].substring(1, 2), '요일약자'), '', '요일');
        const msg_text = msg_split[0].concat(' (', variant_text, ') ', getLocalizeString('설명_주문부터적용됩니다수정하시겠습니까', '', '주문부터 적용됩니다. 수정하시겠습니까?'));
        if (confirm(msg_text)) {
          axios.post('/ajax/regularly/site_update_modify_regularly_address.cm', submit_data).then(res => {
            if (res.status === 200) {
    ...
    $.ajax({
      ...
      success: function(res) {
        $.cocoaDialog.open({
          ...
          SITE_SHOP_MYPAGE.renderRegularlyAddressModalContents({
            'addOrList': props[0].addOrList
          });
          SITE_SHOP_MYPAGE.toggleRegularAddressListAddSection({
            'addOrList': props[0].addOrList
          });
  ...
  const updateRegularlyAddressAjax = () => {
    ...
    update_regularly_address_btn.addEventListener('click', () => {
      ...
      JS_DELAY('postDelay', function() {
        axios.post('/ajax/regularly/site_update_modify_regularly_address.cm', check_regularly_shipping_data).then(res => {
          console.log(res);
          if (res.status === 200) {
            if (res.data.msg === 'SUCCESS') {
              ...
              if (res.data.confirm_msg) {
                ...
                const msg_text = msg_split[0].concat(' (', variant_text, ') ', getLocalizeString('설명_주문부터적용됩니다수정하시겠습니까', '', '주문부터 적용됩니다. 수정하시겠습니까?'));
                if (confirm(msg_text)) {
                  axios.post('/ajax/regularly/site_update_modify_regularly_address.cm', add_regularly_shipping_data).then(res => {
                    if ($("input[name=is_add_shipping]:checked").val() === 'Y') {
  ...
  return {
    ...
    C3_withdrawExchangeOrder: function({
      orderCode,
      orderSectionCode,
      orderSectionItemCode,
      qty
    }) {
      omsWithdrawReturnOrder({
        orderCode,
        orderSectionCode,
        orderSectionItemCode,
        qty,
    ...
    C3_withdrawReturnOrder: function({
      orderCode,
      orderSectionCode,
      orderSectionItemCode,
      qty
    ...
    C3_withdrawCancelOrder: function({
      orderCode,
      orderSectionCode,
      orderSectionItemCode,
      qty
    }) {
      omsWithdrawCancelOrder({
        orderCode,
        orderSectionCode,
        orderSectionItemCode,
        qty
