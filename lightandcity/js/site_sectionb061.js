$(function() {
      if (LIMIT_API_LIST.indexOf('youtube') === -1) {
        ...
        if ($youtube_player_api.length === 0) {
          $youtube_player_api = $('<script src="https:
            var $first_script = $('script').eq(0); $first_script.before($youtube_player_api);
          }
        }
      });
    var SITE_SECTION_YOUTUBE = function() {
      ...
      var youTubePlayer = function() {
        $section_obj.imagesLoaded().always(function() {
          tv = new YT.Player(screen_code, {
            events: {
    ...
    var SITE_SECTION_VIMEO = function() {
      ...
      var vidRescale = function() {
        ...
        if (w / h > org_width / org_height) {
          ...
          $screen.css({
            'left': -(($screen_iframe.outerWidth() - w) / 2) - 10,
            "top": "-10px"
          });
        }
      };
    ...
    var SECTION_RESIZE = function() {
      ...
      var calc = function() {
        ...
        if ($aside.css('display') == 'none') return;
    ...
    var SECTION_PARTICLE_ANIMATION = function() {
      ...
      var init = function(c, vc) {
        ...
        if ($section_class.length == 0) return;
        setTimeout(function() {
          run();
        }, 1000);
        $(window).on('resize', function() {
      ...
      function clearAnimation() {
        ...
        if (typeof renderer !== 'undefined' && typeof renderer.domElement !== 'undefined') $(renderer.domElement).remove();
      }
      ...
      function run() {
        ...
        var material = new THREE_IMWEB.ParticleBasicMaterial({
          map: new THREE_IMWEB.Texture(particleImage)
        });
        for (var i = 0; i < 150; i++) {
          ...
          if (SCREEN_HEIGHT < 1000) particle.scale.x = particle.scale.y = 1.5;
          else particle.scale.x = particle.scale.y = 1;
          scene.add(particle);
    ...
    var SECTION_FIXED = function() {
      ...
      var limit = 0;
      var margin_top = 0;
      var fixed_class = 'new_fixed_menu_on';
      ...
      var getNextId = function($section, type) {
        ...
        if (typeof next_id === 'undefined') return false;
        return next_id;
      };
      return {
        'init': function(c, d) {
