...
function redirectToLoginPage() {
  var back_url_base64 = window.location.href;
...
var SITE_SHOP_DETAIL = function() {
  ...
  var selected_options = [];
  var selected_prod_additional_options = [];
  var selected_require_options = [];
  var selected_prod_additional_require_options = [];
  var current_select_additional_prod_idx = 0;
  ...
  var total_price_localize_text = '';
  var add_order_progress_check = false;
  var use_lazy_load = true;
  var tab_type = 'Y';
  ...
  const setImageWidthHeightBeforeLoad = async (node) => {
    ...
    const loadingImageJobs = Array.prototype.map.call(imgElements, (async (imgElement) => {
      ...
      try {
        ...
        const signal = typeof AbortSignal.timeout === 'function' ? AbortSignal.timeout(timeoutLimit) : new AbortController()
        if (typeof signal.abort === 'function') {
          abortTimer = setTimeout(signal.abort, timeoutLimit);
        }
        const imgRes = await fetch(srcResized, {
          method: 'HEAD',
          signal
        });
        const checkResized = imgRes.headers.has(ORIGINAL_WIDTH_HEADER_KEY) && imgRes.headers.has(ORIGINAL_HEIGHT_HEADER_KEY);
        if (!imgRes.ok || !checkResized) {
          throw new Error();
        }
        const meta = {
        ...
        imgElement.srcset = [1536, 1280, 1080, 828, 768, 640, 576, 368].map((w) => {
  ...
  var initDetail = function(_option) {
    ...
    if (is_prod_detail_page) {
      ...
      $prod_additional_options_default = $prod_additional_options;
  ...
  var runLazyload = function() {
    ...
    if (is_mobile_width) {
      ...
      $prod_detail_content_mobile.find(lazy_load_selector).lazyload({
      ...
      $prod_detail_content_pc.find(lazy_load_selector).lazyload({
        placeholder: NO_IMAGE_URL,
        threshold: 100,
        effect: "fadeIn",
        load: function() {
  ...
  var setImgZoom = function(margin, max_width, section_code) {
    if ($('.shop_view .xzoom').length > 0) {
      var shop_view_xzoom = $('.shop_view .xzoom, .shop_view .xzoom-gallery').xzoom({
        Xoffset: margin,
        openOnSmall: false,
        defaultScale: 0,
        scroll: false,
  ...
  var getProdWish = function(prod_code) {
    $.ajax({
      ...
      dataType: 'json',
      success: function(res) {
        if (res.msg == 'SUCCESS') {} else alert(res.msg);
      }
    });
  };
            ...
            image.onload = function() {
              ...
              setTimeout(function() {
                ...
                if (that.src === first_url) {
                  $prod_image_list.find('.owl-carousel').css({
                    'width': `${new_width}px`,
                    'height': `${new_height}px`
                  });
                  $prod_image_list.find('.owl-height').css({
                    'height': `${new_height}px`
                  });
                }
              }, 5);
    ...
    $.ajax({
      ...
      success: function(result) {
        if (result.msg === 'SUCCESS') {
          ...
          document.dispatchEvent(new CustomEvent('productDetailLoaded', {
            detail: {
              idx
            }
          }));
  ...
  var loadProdAdditional = function(prod_idx) {
    if (is_mobile_width) {
      $.ajax({
        ...
        data: {
          'prod_idx': prod_idx,
          'selected_additional_prod_idx': current_select_additional_prod_idx,
          '__': prod_edit_time
        },
        url: ('/shop/load_prod_additional_mobile.cm'),
        dataType: 'json',
        cache: false,
        success: function(result) {
          if (result.msg == 'SUCCESS') {
    ...
    if (is_mobile_width) {
      $.ajax({
        ...
        data: {
          ...
          'selected_require_options': selected_prod_additional_require_options,
          '__': prod_edit_time,
          'is_additional': true
        },
    ...
    $('._prod_additional_option_optional').each((k, v) => {
      ...
      if ($that.attr('data-prod-idx') > 0) {
        $.ajax({
          type: 'POST',
          data: {
            'prod_idx': $that.attr('data-prod-idx'),
            '__': prod_edit_time,
            'is_additional': true
          },
  ...
  var changeProdAdditionalRequireInputOption = function(type, prod_idx, option_code, msg, success) {
    ...
    if (selected_prod_additional_require_options.length == prod_additional_require_option_count[prod_idx]) {
      selectProdAdditionalOption(prod_idx, selected_prod_additional_require_options, true, 1, function() {
        ...
        $('#prod_additional_drop_menu_content').text(getLocalizeString('타이틀_추가상품', '', '추가 상품'));
        try {
          $('._prod_additional_inline_options').remove();
          $prod_additional_options = $prod_additional_options_default;
        } catch (e) {}
        $('#btnProdAdditionalSelect').text(getLocalizeString('타이틀_추가상품', '', '추가 상품'));
        success();
      }, function(msg) {
        alert(msg);
      });
  ...
  var selectOption = function(prod_idx, {
    options,
    require,
    count,
    idx = 0
        ...
        if (result.msg == 'SUCCESS') {
          ...
          if (no == -1) {
            ...
            if (selected_prod_data.maximum_purchase_quantity == 0 || total_selected_count < selected_prod_data.maximum_purchase_quantity) {
              selected_prod_additional_options.push({
                prod_idx,
                prod_name,
  ...
  var checkRequireOption = function() {
    if ($('._requireOption').length > 0 || $('._requireInputOption').length > 0) {
      if ($('._selected_require_option').length == 0) return false;
    }
    return true;
  };
  ...
  var checkOptionCount = function(optNo, cnt) {
    ...
    if (selected_options[optNo].require) {
  ...
  var checkProdAdditionalOptionCount = function(optNo, cnt) {
    ...
    if (selected_prod_additional_options[optNo].require) {
      if (prod_type === SHOP_CONST.PROD_TYPE.DIGITAL) {
  ...
  var increaseOptionCount = function(optNo, success) {
    ...
    else curCount = parseInt(curCount) + 1;
    ...
    if (max_prod_quantity > 0) {
      ...
      } else if (maximum_purchase_quantity_type !== 'order') {
  ...
  var increaseProdAdditionalOptionCount = function(optNo, success) {
    ...
    if (selected_prod_data.maximum_purchase_quantity > 0) {
      var _selected_count = 0;
      if (selected_prod_data.maximum_purchase_quantity_type === 'order') {
        $.each(selected_prod_additional_options, function(idx, obj) {
          _selected_count += obj.count;
        });
  ...
  var updateSelectedOptions = function(type) {
    ...
    var membership_discount_not_involved_price = 0;
    ...
    if (is_use_prod_additional) {
      ...
      $.each(selected_prod_additional_options, function(no, data) {
        ...
        if (typeof prod_data === 'undefined') return true;
    ...
    $.ajax({
      ...
      success: function(result) {
        if (result.msg == 'SUCCESS') {
          ...
          fetch('/ajax/oms/OMS_auth.cm').then((res) => res.json()).then((res) => {
            if (res.msg === 'SUCCESS') {
      ...
      callback: function(result) {
        switch (type) {
          ...
          default:
            ...
            if (is_guest_login && cm_data.use_login_popup === 'N' && !options.is_using_nomember_order_login_after) {
              if (result.code === 11) {
        ...
        switch (type) {
          ...
          default:
            ...
            if (is_guest_login && cm_data.use_login_popup === 'N' && !options.is_using_nomember_order_login_after) {
              ...
              window.location.href = "/shop_payment/?order_code=" + encodeURIComponent(result.c3_result.orderCode) + "&order_no=" + encodeURIComponent(result.c3_result.orderNo) + (result.c3_result.is_member ? '' : `&gt=${result.c3_result.memberCode}`);
            }
            break;
        }
      }
  ...
  var addNPayWish = function() {
    $.ajax({
      type: 'POST',
      data: {
        'prod_idx_list': [current_prod_idx]
      },
      ...
      success: function(result) {
        if (result.msg == 'SUCCESS') {
          if (result.mobile == 'Y') window.location.href = result.npay_url;
          else window.open(result.npay_url, "", "scrollbars=yes,width=400,height=267");
        } else {
      ...
      if (cm_data.use_login_popup === 'Y') {
        window.SITE_MEMBER.openLogin(backUrl, 'payment', function() {
          if (window.USE_OMS === true) {
            OMS_addOrder('normal', encodeURIComponent(base64Encode(window.location.href)))
          } else {
            addOrder('normal', encodeURIComponent(base64Encode(window.location.href)));
          }
        }, 'N', 'payment', );
      } else {
      ...
      switch (true) {
        case use_cdn_optimized && (type === SHOP_CONST.TAB_TYPE.DETAIL): {
          ...
          if (typeof shop_view_body_width != 'undefined') {
            $prod_detail_content_pc.css({
              'width': shop_view_body_width + '%'
            });
          }
        ...
        case type === SHOP_CONST.TAB_TYPE.REVIEW: {
          ...
          $.ajax({
            ...
            data: {
              ...
              'qna_page': qna_page,
              'only_photo': only_photo,
              'is_mobile': SHOP_CONST.IS_MOBILE,
              'rating': rating
            },
            url: ('/shop/prod_review_pc_html.cm'),
            dataType: 'html',
            ...
            success: function(result) {
              ...
              if (tab_type === 'Y') {
                ...
                $prod_detail_content_pc.html(result).css({
                  'width': '100%'
                });
              }
            }
        ...
        case type === SHOP_CONST.TAB_TYPE.RETURN: {
          ...
          if (last_refund_data) {
            ...
            $_target.empty();
            $_target.html(prod_detail_return_html).css({
              'width': '100%'
            });
          }
          $prod_detail_content_pc.toggleClass('product_detail', false);
      ...
      switch (true) {
        case use_cdn_optimized && (type === SHOP_CONST.TAB_TYPE.DETAIL): {
          ...
          loadTemplate('prodDetailMobile', $prod_detail_content_mobile, async (node) => {
            ...
            if (node.children.length > 0) {
              if (window.IS_MOBILE && $('#prod_deliv_setting .countryList').length == 1) {
        ...
        case type === SHOP_CONST.TAB_TYPE.DETAIL: {
          ...
          if (mobile_detail_html) {
            ...
            if (IS_MOBILE && $('#prod_deliv_setting .countryList').length == 1) {
              loadDelivSetting(current_prod_idx, $('#prod_deliv_setting .countryList').val(), $('#deliv_type').val(), $('#deliv_pay_type').val());
            }
          } else {
            $prod_detail_content_mobile.html('<div style="text-align: center; padding: 50px 0;"><div class="body_font_color_40" style="font-size: 18px; margin:30px"><div> </div>');
          ...
          $.ajax({
            ...
            success: function(result) {
              ...
              if (tab_type === 'Y') {
                ...
                $prod_detail_content_mobile.html(result).css({
                  'width': '100%'
                });
              }
              $prod_detail_content_mobile.toggleClass('product_detail_mobile', false);
  ...
  var openProdDetailFromShoppingList = function(idx, back_url, is_prod_detail_page, is_mobile, prod_idx_org) {
    $.ajax({
      ...
      data: {
        'idx': idx,
        'is_prod_detail_page': is_prod_detail_page
      },
      ...
      success: function(result) {
        $.cocoaDialog.open({
          type: 'prod_detail_from_shopping_list',
          custom_popup: result,
          'close_block': false
        });
        ...
        $('.modal_prod_detail_from_shopping_list').on('hidden.bs.modal', function(e) {
          $('.modal_prod_detail_from_shopping_list').find('.modal-content').html('');
          $('html').removeClass('mobile-shop-open');
  ...
  var openRequireSelect = function(option_code) {
    ...
    var top = Math.floor($form_select.offset().top);
    var scroll_top = Math.floor($goods_wrap.scrollTop());
    var position_top = $('#option_' + option_code).position().top;
    var height = Math.floor($('.opt-group').height());
    var default_value = 180;
    $form_select.parent('._form_parent').nextAll().toggleClass('disabled', true);
    if (top - height >= default_value) {
      $goods_wrap.animate({
        scrollTop: position_top - 50
      }, 0);
      if (scroll_top >= default_value) {
        $goods_wrap.animate({
          scrollTop: position_top + $('.option_box_wrap').outerHeight()
        }, 0);
      }
    }
  };
  ...
  async function selectFreebieAsync() {
    ...
    const items = [...(selected_options.filter(optionSelected => optionSelected.require).length === 0 ? [{
      prodIdx: current_prod_idx,
      prodCode: null,
      optionDetailCode: "",
      prodOptions: [],
    ...
    const item_additional = selected_prod_additionals.flatMap((additional) => {
      const additionalOptions = selected_prod_additional_options.filter(option => option.prod_idx === additional.idx);
    ...
    return new Promise((resolve) => {
      document.dispatchEvent(new CustomEvent('@im/fo-prod-detail-freebie:openDialogSelectFreebie', {
        detail: {
          callback: resolve,
  ...
  function confirmOrderWithCartItems(type, backurl, params) {
    ...
    if (!options.is_price_view_permission && window.IS_GUEST && cm_data.use_login_popup === 'Y' && options.is_using_nomember_order_login_after) {
      window.SITE_MEMBER.openLogin(encodeURIComponent(base64Encode(window.location.href)), 'payment', _confirmOrderWithCartItems, 'N', 'payment', );
    } else {
    ...
    function proceedToOrder() {
      if (typeof params.onMoveOrder === 'function') {
        ...
        if (type === 'guest_login' || type === 'npay' || type === 'talkpay') {
          ...
          selectFreebieAsync().then((selected_freebies) => {
  ...
  function getThumbnailBadgeStyleByPosition({
    badgeColor,
    badgePosition,
    imageWidth = ''
  }) {
    ...
    const leftTopFontRatio = 0.035;
    const leftTopPaddingRatio = 0.015;
    const bottomFontRatio = 0.04;
    ...
    leftTopFontSize = Math.max(10, Math.min(13, leftTopFontSize));
    leftTopPadding = Math.max(3, Math.min(6, leftTopPadding));
    bottomFontSize = Math.max(12, Math.min(14, bottomFontSize));
    bottomPadding = Math.max(4, Math.min(8, bottomPadding));
    const badgeLeftTopGapSize = roundTo3(leftTopFontSize * (8 / 28));
    if (badgePosition === BADGE_POSITION_LEFT_TOP) {
      const style = ['position: absolute', 'border-radius: 3px', `top: ${badgeLeftTopGapSize}px`, `left: ${badgeLeftTopGapSize}px`, `padding: ${leftTopPadding}px`, 'color: #FFF', 'vertical-align: middle', 'display: inline-flex', 'align-items: center', 'justify-content: center', `background-color: ${badgeColor}`, 'z-index: 10', `font-size: ${leftTopFontSize}px;`].join('; ');
      return style;
    }
    if (badgePosition === BADGE_POSITION_BOTTOM) {
      const style = ['position: absolute', 'left: 0', 'bottom: 0', 'width: 100%', `padding: ${bottomPadding}px`, 'color: #FFF', 'vertical-align: middle', 'display: inline-flex', 'align-items: center', 'justify-content: center', `background-color: ${badgeColor}`, 'z-index: 10', `font-size: ${bottomFontSize}px;`].join('; ');
      return style;
    }
    return '';
  }
  ...
  return {
    ...
    selectOption: function(prod_idx, {
      options,
      require,
      count,
      idx = null
    ...
    selectAdditionalProduct: (product_idx, price, name, maximum_purchase_quantity, member_maximum_purchase_quantity, maximum_purchase_quantity_type, optional_limit, optional_limit_type, stock_use, stock, stock_unlimit, option_mix_type, prod_type, period_discount_flag, period_discount_data, period_discout_group_list, membership_discount) => {
      ...
      if (is_mobile_width) {
        loadOption('prod', current_prod_idx);
    ...
    getThumbnailBadgeStyleByPosition: function({
      badgeColor,
      badgePosition,
      imageWidth = ''
    }) {
      return getThumbnailBadgeStyleByPosition({
        badgeColor,
        badgePosition,
        imageWidth
      });
...
var SITE_SHOP_CART = function() {
  ...
  var loadProdWishList = function() {
    ...
    $.ajax({
      ...
      data: {
        'type': 'cart',
        'is_design_mode': is_design_mode ? 'Y' : 'N'
      },
  ...
  var addOrderWithCart = async function(type, item_code, backurl, direct, is_oms, params, is_gift_buy) {
    ...
    var item_code_list = [];
    if (item_code == '') item_code_list = selectedCartItem;
    else item_code_list = [item_code];
    ...
    const {
      freebiesUserSelected,
      selectedAllFreebiesRequired
    } = await new Promise((resolve) => {
      if (!is_oms || type === 'regularly') {
        resolve({
          freebiesUserSelected: [],
          selectedAllFreebiesRequired: true
        });
        return;
      }
      if (type === 'npay' || type === 'talkpay') {
  ...
  var addNPayWishWithCart = function(item_code) {
    ...
    $.ajax({
      ...
      success: function(result) {
        if (result.msg == 'SUCCESS') {
          if (result.npay_url == '') alert(LOCALIZE.설명_네이버페이찜등록실패(escape_javascript(result.errmsg)));
          else {
            if (result.mobile == 'Y') window.location.href = result.npay_url;
            else window.open(result.npay_url);
          }
        } else {
          alert(result.msg);
        }
      ...
      success: function(result) {
        if (result.msg == 'SUCCESS') {
          ...
          $.each(result.selected_option_list, function(no, data) {
            SITE_SHOP_DETAIL.selectOption(result.prodIdx, {
...
var SITE_SHOP_REVIEW = function() {
  ...
  var openAddReview = function(idx, prod_code, prod_order_code, is_c3 = false) {
    ...
    $.ajax({
      ...
      data: {
        'idx': idx,
        'prod_code': prod_code,
        'prod_order_code': prod_order_code
      },
  ...
  function resizeStickyContainer() {
    ...
    if ($(window).width() >= 768) {
      if ($review_container.hasClass('bg_on')) $review_container.find('#toolbarContainer').toggleClass('pc_sticky_toolbar', s_top > 487);
      else $review_container.find('#toolbarContainer').toggleClass('pc_sticky_toolbar', s_top > 180);
    }
  }
          ...
          if (confirm(msg)) {
            $.ajax({
              ...
              data: {
                code: t,
                prod_code: c,
                change_member_point: res.change_member_point
              },
  ...
  var EditReviewShow = function(t, c, idx) {
    ...
    $show_secret_password.off('click', '._confirm').on('click', '._confirm', function() {
    ...
    $('body').off('mousedown.show_secret').on('mousedown.show_secret', function(e) {
  ...
  var reviewCompleted = function(received_point, check_received_point) {
    ...
    $.ajax({
      ...
      data: {
        ...
        'check_received_point': check_received_point
      },
  ...
  var switchToBlockCancelButton = function(login_member_code, review_code, review_member_code, type) {
    ...
    $('._review_button._block_' + review_member_code).text(getLocalizeString("설명_차단해제", "", "차단해제")).attr("onclick", onclick);
  };
  var switchToBlockCancelBody = function(review_member_code) {
    $('._review_body._block_' + review_member_code).html("<p class='text-gray'><i aria-hidden='true' class='icon icon-exclamation' style='margin-right: 5px;'></i>" + getLocalizeString("설명_차단한작성자의구매평입니다", "", "차단한 작성자의 구매평입니다.") + "</p>").attr('block', 'true');
    $('._review_img._block_' + review_member_code).css('display', 'none');
    $('._block_no_img._block_' + review_member_code).css('display', 'block');
  };
  ...
  var switchToBlockButton = function(login_member_code, review_code, review_member_code, type) {
    ...
    $('._review_button._block_' + review_member_code).text(getLocalizeString("설명_차단", "", "차단")).attr("onclick", "SITE_SHOP_REVIEW.addBlockPost('" + login_member_code + "','" + review_code + "'); event.stopPropagation();");
  };
  var switchToBlockBody = function(review_member_code, block_review_list_code, block_review_data) {
    ...
    block_review_list_code.forEach(block_review_code => {
      $('._review_body._review_code_' + block_review_code).html(block_review_data[block_review_code]['body'].replace(/(\n|\r\n)/g, '<br>')).attr('block', 'false');
    });
    $('._block_no_img._block_' + review_member_code).css('display', 'none');
  };
...
var SITE_SHOP_QNA = function() {
  ...
  function resizeStickyContainer() {
    ...
    if ($(window).width() >= 768) {
      if ($qna_container.hasClass('bg_on')) $qna_container.find('#toolbarContainer').toggleClass('pc_sticky_toolbar', s_top > 487);
      else $qna_container.find('#toolbarContainer').toggleClass('pc_sticky_toolbar', s_top > 180);
    }
  }
    ...
    $.ajax({
      ...
      success: function(result) {
        if (result.msg == 'SUCCESS') {
          ...
          $('._comment_body_' + idx).closest('.comment_area').find('#_show_content_btn').hide();
  ...
  var submit = function() {
    if (IE_VERSION < 10) {
      ...
      if (qna_body.hasClass('fr-code-view')) FroalaEditor('#qna_body').codeView.toggle();
